...
export class GAME_4_CreditCollectResultCommand extends puremvc.MacroCommand {
    ...
    protected sendCreditCollectNotification(array: Array<number>){
        ...
        .registerTimer('CreditCollect'+this.allCount, this.intervalTime*this.allCount,
         () => {
            this.curCount++;
            this.sendNotification(ViewMediatorEvent.COLLECT_CREDIT_BALL,array);
            GlobalTimer.getInstance().removeTimer('CreditCollect'+this.curCount);
    ...
    protected checkShowResultBoard(){
        if(this.curCount>=this.allCount){
            GlobalTimer.getInstance()
            .registerTimer('checkGameEnd', this.intervalTime + 1,
            this.showCreditBoard, this)
            .start();
        }  
    }
    ...
    protected showCreditBoard() {
        ...
        let totalWin: number = this.gameDataProxy.convertCredit2Cash(
            this.gameDataProxy.spinEventData.topUpGameResult.topUpGameTotalWin + this.gameDataProxy.spinEventData.baseGameResult.baseGameTotalWin
        );
