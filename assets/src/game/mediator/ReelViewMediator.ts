...
export class ReelViewMediator extends BaseReelViewMediator<GAME_ReelView> {
    ...
    public handleNotification(notification: puremvc.INotification): void {
        ...
        switch (name) {
            ...
            case ReelEvent.SHOW_STACK_WILD:
                ...
                break;
            case ReelEvent.SHOW_MYSTERY:
                this.showMystery(notification.getBody());
                break;
                ...
                for (let i = 0; i < this.reelView.reelsList.length; i++) {
                    ...
                    content.multipleWeight = this.getWeight(
                        topUpextendSetting.ballRateHitWeight[
                            this.gameDataProxy.spinEventData.baseGameResult.extendInfoForbaseGameResult.ballCount - 6
                        ]
                    );

                    content.addRoundArray = topUpextendSetting.addRoundPerHit;
                    content.addRoundWeight = this.getWeight(topUpextendSetting.addRoundWeight);
                }
                this.setSymbolPosData();
                break;
        }
    ...
    protected showMystery(symbolInfos: Array<SymbolInfo>) {
        this.reelView.createAnimSymbols(symbolInfos);
        for (let i = 0; i < symbolInfos.length; i++) {
            const info = symbolInfos[i];
            this.reelView.showMysterySymbol(info, this.reelDataProxy.symbolFeature[info.x][info.y]);
        }
    }
